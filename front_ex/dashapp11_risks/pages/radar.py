...
def get_circl_info(resize_balles,bar_theta,bar_r,bar_bound,risks, max_p_rank,max_group_rank,max_dam_rank): # Вся инфо по размещению шариков
#     trial_1_r = []
#     trial_1_theta=[]
#     marker_size=[]
#     ball_text =[]
...
#                                 set_trace()
                                ...
                                ball_text0.append(int(n))
                                hover_text0.append(risk_name)
                                bar_width[-1]=bar_width[-1]+2*size_marker
                                if bar_width[-1]==24:
                                    print(n)
                            else: # Не первый в ряду
        #                         print(trial_1_theta0)
...
#                         set_trace() 
                        lvl=lvl+1
                        bar_width=bar_width+[0]
            bar_width_all.append(bar_width)
            lvl=1
            is_new=False  
            ball_text=ball_text+ball_text0
            hover_text=hover_text+hover_text0
        ...
        bar_width_all=np.array(bar_width_all)
#         set_trace()
        bar_bound=[0]+np.cumsum(bar_width_all.max(axis=0))[:-2].tolist()
        
        bar_r=[]
        for i in bar_width_all.max(axis=0)[:-2]:
            bar_r=bar_r+max_group_rank*[i]
        bar_r=bar_r+max_group_rank*[100-bar_width_all.max(axis=0)[:-2].sum()]
        if (not is_new_prob) and (np.abs(np.sum(bar_bound0)-np.sum(bar_bound))<1):
            break
        is_new_prob=False
        bar_bound0=bar_bound
    return ball_text,trial_1_r,trial_1_theta,marker_size,hover_text, bar_bound,bar_r
...
def main0(risks0,  bigest_size,b,koef,koef_r,try_resize_radius_bounds,resize_balles, masks=None):
    ...
    while True:
        ...
        ball_text,trial_1_r,trial_1_theta,marker_size,hover_text, bar_bound,bar_r=get_circl_info(resize_balles, bar_theta,bar_r,bar_bound,risks, max_p_rank,max_group_rank,max_dam_rank)
