...
def get_resellers_count(start_date, end_date, branches, gruz, rod):
    ...
    return pd.read_sql(sql, con=create_engine(config.POSTGRE_DB,
...
def get_resellers_share(start_date, end_date, branches, gruz, rod):
    ...
    sql2 = '''
        ...
        AND "Род подвижного состава" IN %s
    ''' % (start_date, end_date, branches, gruz, rod)
    df1 = pd.read_sql(sql1, con=create_engine(config.POSTGRE_DB, max_identifier_length=128, encoding='utf-8'))
    df2 = pd.read_sql(sql2, con=create_engine(config.POSTGRE_DB, max_identifier_length=128, encoding='utf-8'))
