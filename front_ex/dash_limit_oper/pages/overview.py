...
def create_layout(app, start_date = None, end_date=None, debug=False):  
    ...
    layout = html.Div([
        ...
        dbc.Row(
            dbc.Col(html.Div(id='graph3'), width=12)
        ),
        dbc.Row(
            html.H4('    По дате документа')
        ),
        dbc.Row(
            html.H6('      С учетом сторно, рублей')
        ),
        dbc.Row(
            dbc.Col(html.Div(id='graph6'), width=12)
        ),
        dbc.Row(
            dbc.Col(html.H6('      Сторно исключено, рублей'), width=12)
        ),
        dbc.Row(
            dbc.Col(html.Div(id='graph5'), width=12)
        )
    ...
    def dogovor(klient):
        ...
        """ % klient
        df6=pd.read_sql(query3, connection_hana)
