...
def get_tors(start_date, end_date):
    ...
    sql = '''
        SELECT
        a.yeardata "Год"
        ,count(a.zakaz)/3 "Общее количество"
        ,c.nekach "Некачественные"
        ,count(a.zakaz)/3-c.nekach "Качественные"
        FROM sap_s4.tor_all_ik a
        left join 
        (select
            a.yeardatadk
            ,count(a.zakazdk)/3 as nekach
...
def get_tors_by_contr(start_date, end_date):
    ...
    sql = '''
        ...
        (
            ...
            ,a.lifnr
            ,a.name1
            ,count(a.zakaz)/3 as total
            ,c.nekach
            ,count(a.zakaz)/3-c.nekach as kach
            from sap_s4.tor_all_ik a
            left join 
                (select
                a.yeardatadk
                ,a.lifnrdk
                ,count(a.zakazdk)/3 as nekach
                from sap_s4.tor_neis_ik a group by a.yeardatadk,a.lifnrdk) c on c.yeardatadk = a.yeardata and c.lifnrdk = a.lifnr
        where a.data1353 between '%s' AND '%s'
        group by a.yeardata, c.nekach, a.lifnr, a.name1) k
    ''' % (start_date, end_date)
...
def get_tors_count(start_date, end_date):
     ...
     sql = '''
         SELECT 
         count(a.ZAKAZ)/3 AS "Количество ремонтов"
         FROM sap_s4.tor_all_ik a
         WHERE a.data1353 BETWEEN '%s' AND '%s'
     ''' % (start_date, end_date)
...
def get_tors_count_nk(start_date, end_date):
     ...
     sql = '''
         SELECT 
         count(a.zakazdk)/3 AS "Количество некачественных ремонтов"
